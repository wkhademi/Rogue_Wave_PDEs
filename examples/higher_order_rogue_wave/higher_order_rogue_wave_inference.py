"""
Data-driven inference of Higher-Order Rogue Wave using Neural Networks.
...
class PhysicsInformedNN:
    ...
    def predict(self, X_star):
        ...
        for idx in range(batches+1):
            ...
            end_idx = min(batch_size*(idx+1), X_star.shape[0])
            tf_dict = {self.x0_tf: X_star[start_idx:end_idx,0:1], self.t0_tf: X_star[start_idx:end_idx,1:2]}
            u_star.append(self.sess.run(self.u0_pred, tf_dict))
            v_star.append(self.sess.run(self.v0_pred, tf_dict))

            tf_dict = {self.x_f_tf: X_star[start_idx:end_idx,0:1], self.t_f_tf: X_star[start_idx:end_idx,1:2]}
            f_u_star.append(self.sess.run(self.f_u_pred, tf_dict))
            f_v_star.append(self.sess.run(self.f_v_pred, tf_dict))

        u_star = np.concatenate(u_star)
        v_star = np.concatenate(v_star)
        f_u_star = np.concatenate(f_u_star)
        f_v_star = np.concatenate(f_v_star)
...
if __name__ == "__main__":
    ...
    with open('psi2_high_order_rogue_wave_data.npz', 'rb') as solution_file:
        data = np.load(solution_file)
        t = data['t'][100:501:2,None]  # t in [-2,2]
        x = data['x'][1850:2151,None]  # x in [-7.5, 7.5]
        Exact = data['U'][100:501:2,1850:2151]
    ...
    plt.ylabel('$t$')
    plt.colorbar()
    plt.savefig("extended_predicted_higher_order_rogue_wave.png")
    plt.show()
    ...
    plt.savefig("extended_predicited_vs_exact_higher_order_rogue_wave.png")
    plt.show()

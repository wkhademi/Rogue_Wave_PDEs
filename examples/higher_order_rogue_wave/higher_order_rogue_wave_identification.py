"""
Data-driven identification of Higher-Order Rogue Wave using Neural Networks.
...
class PhysicsInformedNN:
    ...
    def train(self, nIter):
        ...
        #                        feed_dict = tf_dict,
        #                        fetches = [self.loss, tf.exp(self.lambda_1), self.lambda_2],
...
if __name__ == "__main__":
    ...
    with open('psi1_high_order_rogue_wave_data.npz', 'rb') as solution_file:
        data = np.load(solution_file)
        t = data['t'][150:451,None]  # t in [-1.5, 1.5]
        x = data['x'][1900:2101,None]  # x in [-6, 6]
        Exact = data['U'][150:451,1900:2101]
    ...
    X_train = X_star[30150+idx,:]
    u_train = u_star[30150+idx,:]
    v_train = v_star[30150+idx,:]

    #idx = np.arange(N) * (X_star.shape[0] // N)
    idx = np.random.choice(X_star.shape[0], N, replace=False)
    X_train = np.concatenate((X_train, X_star[idx,:]))
    u_train = np.concatenate((u_train, u_star[idx,:]))
    v_train = np.concatenate((v_train, v_star[idx,:]))
    #X_train = X_star[idx,:]
    #u_train = u_star[idx,:]
    #v_train = v_star[idx,:]
    ...
    error_H = np.linalg.norm(Exact_h[150,:]-H_pred[150,:],2)/np.linalg.norm(Exact_h[150,:],2)
    ...
    plt.ylabel('$t$')
    plt.colorbar()
    plt.savefig("psi1_predicted_higher_order_rogue_wave_identification.png")
    plt.show()

    plt.plot(x, Exact_h[150,:], 'b-', linewidth = 2, label = 'Exact')
    plt.plot(x, H_pred[150,:], 'r--', linewidth = 2, label = 'Prediction')
    plt.title('$t = 0$', fontsize=10)
    plt.xlabel('$x$')
    plt.ylabel('$|h(x,t)|$')
    plt.legend(frameon=False)
    plt.savefig("psi1_predicted_vs_exact_higher_order_rogue_wave_identification.png")
    plt.show()

...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, v0, X_f, layers, lb, ub, N, m):
        ...
        t0 = np.stack((np.full_like(x0, 0.0), np.full_like(x0, 2.*np.pi / 5.), np.full_like(x0, 4.*np.pi / 5.), np.full_like(x0, 6.*np.pi / 5.)), axis=0)[:,:,0]
        x0 = np.stack((x0, x0, x0, x0), axis=0)[:,:,0]
        X0 = np.hstack((x0.flatten()[:, None], t0.flatten()[:, None]))
        ...
        self.x_f = np.concatenate((self.x0, X_f[:,0:1]), axis=0)
        self.t_f = np.concatenate((self.t0, X_f[:,1:2]), axis=0)
...
if __name__ == "__main__":
    ...
    # initial condition
    x0 = x[:,None]
    u0 = np.concatenate((Exact_u[0:1,:], Exact_u[1000:1001,:], Exact_u[2000:2001,:], Exact_u[3000:3001,:]), axis=0)
    v0 = np.concatenate((Exact_v[0:1,:], Exact_v[1000:1001,:], Exact_v[2000:2001,:], Exact_v[3000:3001,:]), axis=0)

    # collocation points
    idx = np.random.choice(t.shape[0], N, replace=False)
    X_f, T_f = np.meshgrid(x, t[idx])
    X_f = np.hstack((X_f.flatten()[:,None], T_f.flatten()[:,None]))
    u0 = np.concatenate((u0.flatten()[:, None], Exact_u[idx,:].flatten()[:, None]), axis=0)
    v0 = np.concatenate((v0.flatten()[:, None], Exact_v[idx,:].flatten()[:, None]), axis=0)
    ...
    plt.ylabel('$t$')
    plt.colorbar()
    plt.savefig("predicted_discrete_exact_nls.png")
    plt.show()
